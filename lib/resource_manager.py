...
class WorkerPool:
    ...
    def _scale_up(self):
        ...
        current_workers = self.executor._max_workers if hasattr(self.executor, '_max_workers') else self.config.max_workers
        
        if current_workers < self.config.max_workers:
            new_workers = min(
                self.config.max_workers,
                int(current_workers * self.config.scale_up_factor)
            )
            
            logger.info(f"📈 Scaling up {self.config.pool_type.value} pool: {current_workers} → {new_workers} workers")
    ...
    def _scale_down(self):
        ...
        if current_workers > self.config.min_workers:
            new_workers = max(
                ...
                int(current_workers * self.config.scale_down_factor)
            )
            
            logger.info(f"📉 Scaling down {self.config.pool_type.value} pool: {current_workers} → {new_workers} workers")
    ...
    def _update_thread_pool_max_workers(self, new_max_workers: int):
        ...
        logger.debug(f"🔄 Would update {self.config.pool_type.value} pool to {new_max_workers} workers")
...
class ResourceManager:
    ...
    def create_worker_pool(self, pool_type: WorkerType, config: Optional[WorkerPoolConfig] = None) -> WorkerPool:
    ...
    def submit_task(self, pool_type: WorkerType, func: Callable, *args, **kwargs) -> concurrent.futures.Future:
    ...
    def _check_resource_limits(self) -> bool:
        ...
        try:
            ...
            if cpu_percent > self.resource_limits.max_cpu_percent:
                logger.warning(f"⚠️  CPU usage {cpu_percent:.1f}% exceeds limit {self.resource_limits.max_cpu_percent}%")
                return False
            
            if memory.percent > self.resource_limits.max_memory_percent:
                logger.warning(f"⚠️  Memory usage {memory.percent:.1f}% exceeds limit {self.resource_limits.max_memory_percent}%")
                return False
            
            if disk.percent > self.resource_limits.max_disk_usage_percent:
                logger.warning(f"⚠️  Disk usage {disk.percent:.1f}% exceeds limit {self.resource_limits.max_disk_usage_percent}%")
                return False
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.resource_history:
                ...
                avg_cpu = sum(m.cpu_percent for m in self.resource_history[-10:]) / min(10, len(self.resource_history))
                avg_memory = sum(m.memory_percent for m in self.resource_history[-10:]) / min(10, len(self.resource_history))
            else:
                latest = None
