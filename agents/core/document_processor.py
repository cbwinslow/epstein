...
class DocumentProcessorAgent(BaseAgent):
    ...
    def _init_worker_pool(self):
        ...
        try:
            ...
            self.processing_pool = self.resource_manager.create_worker_pool(
                WorkerType.PROCESSING, processing_config
            )
    ...
    async def process_document(
        ...
        try:
            ...
            future = self.processing_pool.submit_task(
                self._process_document_task, document_path, document_id, metadata or {}
            )
    ...
    async def _process_document_task(
        ...
        try:
            ...
            result = await self.pipeline._process_single_document(
                document_path, source_id=metadata.get("source_id", "agent")
            )
    ...
    async def process_batch(
        ...
        try:
            ...
            self.observability.metrics.documents_processed.inc(
                labels={"source": "agent", "status": "success"}, amount=success_count
            )
            self.observability.metrics.documents_processed.inc(
                labels={"source": "agent", "status": "failed"}, amount=failed_count
            )
    ...
    async def extract_text(
        ...
        try:
            ...
            pages_text, page_count, ocr_required = self.pipeline._extract_text_from_document(
                document_path
            )
    ...
    async def analyze_document(
        ...
        try:
            ...
            analysis = await self.pipeline._analyze_with_ai(
                model="gpt-3.5-turbo", document_text=document_text, analysis_type=analysis_type
            )

            return {"analysis_type": analysis_type, "result": analysis, "model": "gpt-3.5-turbo"}
    ...
    async def extract_entities(self, document_text: str, page_number: int = 1) -> dict[str, Any]:
        ...
        try:
            ...
            return {"page_number": page_number, "entities": entities, "entity_count": len(entities)}
        ...
        try:
            if task_type == "process_document":
                return await self.process_document(
                    document_path=payload["document_path"], metadata=payload.get("metadata")
                )
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        agent = create_document_processor_agent(
            "test_processor", {"max_workers": 2, "ocr_enabled": True}
        )
