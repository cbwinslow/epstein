...
class ExtractedEntity:
    ...
    document_id: str
    extracted_text_id: Optional[str]
    entity_type: str
    entity_text: str
...
class EpsteinIngestionPipeline:
    ...
    def _init_directories(self):
        ...
        logger.debug(f"📁 Directories initialized:")
    ...
    def _store_document_metadata(self, metadata: DocumentMetadata) -> Optional[str]:
    ...
    def _store_extracted_text(self, text_data: ExtractedText) -> Optional[str]:
    ...
    async def _process_single_document(self, file_path: str, source_id: str) -> bool:
        ...
        try:
            ...
            if not stored_doc_id:
                logger.warning(f"⚠️  Failed to store document metadata")
    ...
    async def run_pipeline(self, source_dir: Optional[str] = None, source_id: str = "govinfo"):
        ...
        try:
            ...
            for i in range(0, len(documents), self.config.batch_size):
                ...
                # Process batch concurrently (collect exceptions to allow error recovery)
                tasks = [self._process_single_document(doc, source_id) for doc in batch]
                results = await asyncio.gather(*tasks, return_exceptions=True)

                # Handle per-task results: count exceptions as errors, allow other tasks to complete
                for idx, res in enumerate(results):
                    if isinstance(res, Exception):
                        self.error_count += 1
                        logger.error(f"❌ Task failed for {batch[idx]}: {res}")
                    elif res is False:
                        self.error_count += 1
