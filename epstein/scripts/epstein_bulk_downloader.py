...
def verify_download(task: DownloadTask, log: Log) -> Tuple[bool, Optional[str]]:
    dest = task.dest
    if not dest.exists():
    ...
    try:
        ...
        expected_sha = task.meta.get("expected_sha256")
        if expected_sha:
            actual_sha = sha256_file(dest)
            if actual_sha != expected_sha:
                return False, f"SHA-256 mismatch: got {actual_sha}, expected {expected_sha}"

        if task.kind == "zip":
            if header[:4] != b"PK\x03\x04":
                return False, "Invalid ZIP magic bytes"
            ok, err = validate_zip(dest)
            if not ok:
                return False, f"ZIP corruption: {err}"
        elif task.kind == "pdf":
            if header[:5] != b"%PDF-":
                return False, "Invalid PDF header"
            # Check for %%EOF to detect truncation
            with dest.open("rb") as f:
                f.seek(-1024, 2)  # last 1024 bytes
                tail = f.read()
                if b"%%EOF" not in tail:
                    return False, "PDF appears truncated (no %%EOF found)"
...
def execute_task(task: DownloadTask, sess: requests.Session, out_dir: Path, timeout_s: int, max_retries: int, backoff_base_s: float, chunk_bytes: int, dry_run: bool, log: Log) -> DownloadResult:
    ...
    try:
        if task.kind in ("zip", "pdf"):
            ...
            for verify_attempt in range(1, DEFAULT_MAX_VERIFY_RETRIES + 1):
                try:
                    ...
                    if not dry_run:
                        ...
                        verification_ok, verification_error = verify_download(task, log)
                        if verification_ok:
                            break
                    else:
                    ...
                    if verify_attempt == DEFAULT_MAX_VERIFY_RETRIES:
                        raise
                    log.warn(f"Download/verification attempt {verify_attempt} failed for {task.name}: {e}")
                    task.dest.unlink(missing_ok=True)
                    continue
            if not verification_ok:
                raise RuntimeError(f"Verification failed after {DEFAULT_MAX_VERIFY_RETRIES} attempts: {verification_error}")
        ...
        if task.kind == "folder":
            ...
            return DownloadResult(source=task.source, name=task.name, url=task.url, dest=task.dest, kind=task.kind, ok=True, verification_ok=True, meta=task.meta)
        ...
        return DownloadResult(source=task.source, name=task.name, url=task.url, dest=task.dest, kind=task.kind, ok=False, error=str(e), verification_ok=False, verification_error=str(e), meta=task.meta)
