...
class TestMultiAgentOrchestrator:
    ...
    async def test_run_comprehensive_analysis(self, orchestrator, telemetry):
        ...
        with patch.object(orchestrator.agents['vector_db_analyzer'], 'analyze_all_collections') as mock_vector:
            with patch.object(orchestrator.agents['db_troubleshooter'], 'optimize_database') as mock_db:
    ...
    async def test_run_troubleshooting_workflow(self, orchestrator, telemetry):
        ...
        # Mock agent methods
        with patch.object(orchestrator.agents['db_troubleshooter'], 'check_database_health') as mock_health:
            mock_health.return_value = {"database_health": {"connection_status": "healthy"}}
