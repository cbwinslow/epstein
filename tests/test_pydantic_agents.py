...
class TestPydanticAIIntegration:
    ...
    async def test_agent_creation(self):
        ...
        agent = Agent(model="test", system_prompt="You are a test agent")
        assert agent is not None

    @pytest.mark.asyncio
    async def test_agent_with_tools(self):
        """Test agent with custom tools"""
        agent = Agent(model="test", system_prompt="You are a document retrieval agent")
    ...
    async def test_agent_mcp_integration(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.json.return_value = [{"collection_id": "test", "name": "Test Collection"}]
...
class TestDownloadAgent:
    ...
    def test_download_request_model(self):
        ...
        request = DownloadRequest(collection_id="test_collection", destination="/tmp/downloads")
    ...
    async def test_download_agent_tool(self, mock_post):
        ...
        agent = Agent(model="test", system_prompt="Download agent")
        ...
        def download_collection(request: DownloadRequest) -> dict:
            ...
            response = requests.post(
                "http://localhost:8765/download/bulk", json=request.model_dump()
            )
            return response.json()

        # Test download tool
        request = DownloadRequest(collection_id="test", destination="/tmp/test")
...
class TestAgentWorkflows:
    ...
    async def test_discovery_and_download_workflow(self, mock_post, mock_get):
        ...
        mock_get_resp.json.return_value = [
            {"collection_id": "doj_releases", "name": "DOJ Releases", "document_count": 100}
        ]
        ...
        # Mock download response
        mock_post_resp = Mock()
        mock_post_resp.json.return_value = {"task_id": "workflow-test", "status": "queued"}
        mock_post_resp.status_code = 200
        mock_post.return_value = mock_post_resp

        # Create workflow agent
        agent = Agent(model="test", system_prompt="Workflow agent")
        ...
        def initiate_download(collection_id: str) -> dict:
            ...
            return requests.post(
                "http://localhost:8765/download/bulk", json={"collection_id": collection_id}
            ).json()
